...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<
    "generator" | "issueReports" | "usage"
  >("generator");

  const [usagePerFountain, setUsagePerFountain] = useState<
    { waterpoint_id: string; count: number }[]
  >([]);

  const handleDelete = async (id: number) => {
    const confirmed = window.confirm(
      "Ben je zeker dat je deze melding wil verwijderen?"
    );
      ...
      try {
        ...
        const processedFeatures = (data.features || []).map((feature: any) => {
          ...
          if (props.objectid_1 && String(props.objectid_1) !== "0") {
            ...
            finalId = `loc-${String(lat).replace(".", "").slice(0, 8)}-${String(
              lon
            )
              .replace(".", "")
              .slice(0, 8)}`;
        ...
        // 3️⃣ USAGE (DIT MIS JE)
        const { data: usageData, error } = await supabase
          .from("waterpoint_usage_counts") // 👈 DE VIEW
          .select("*");

        if (error) {
          console.error("Usage fetch error:", error);
        } else {
          setUsagePerFountain(usageData || []);
        }
      } catch (error) {
    ...
    if (!search) {
      ...
      const filtered = allLocations.filter((p) => {
        ...
        return (
          ...
          street.toLowerCase().includes(lowerSearch)
        );
      });
  ...
  const getLocationById = (id: string) => {
    return allLocations.find((loc) => loc.properties.fixedId === id);
  };
  return (
    <div className="p-4">
          ...
          <div className="flex-1 flex flex-col">
            ...
            <div className="overflow-y-auto flex-1 border rounded">
              {filteredLocations.map((location, index) => {
                ...
                const address =
                  location.properties.straatnaam ||
                  location.properties.adres ||
                  location.properties.locatie ||
                  "";
                return (
                  <div
                    key={index}
                    className="border-b p-4 hover:bg-blue-50 transition cursor-pointer"
                    onClick={() =>
                      setSelectedQrPoint({
                        id: locationId,
                        naam: locationName,
      ...
      {activeTab === "usage" && (
        <div className="mt-6">
          <h2 className="text-xl font-bold mb-4">Gebruik per waterfontein</h2>

          {usagePerFountain.length === 0 ? (
            <p className="text-gray-500">Nog geen gebruik geregistreerd.</p>
          ) : (
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase">
                    Aantal keer genavigeerd
                  </th>
                </tr>
              </thead>

              <tbody className="bg-white divide-y divide-gray-200">
                {usagePerFountain.map((usage) => {
                  const location = getLocationById(usage.waterpoint_id);

                  return (
                    <tr key={usage.waterpoint_id}>
                      <td className="px-6 py-4">
                        <div className="font-bold">
                          {location?.properties.naam ??
                            location?.properties.beschrijving ??
                            "Onbekende locatie"}
                        </div>
                      ...
                      <td className="px-6 py-4 font-bold">{usage.count}</td>
                    </tr>
                  );
                })}
              </tbody>
